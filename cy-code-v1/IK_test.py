...
def IK(x,y_,z):
    try:
        y = sqrt(x**2 + y_**2) - cl # actual location of tip from femur in coxa frame
        k = sqrt(y**2 + z**2) # shortest distance from femur to tibia tip 
        ...
        theta = asin(max(-1, min(1, z / k))) # angle between horizontal and k
        theta2 = acos(max(-1, min(1, ((fl**2)+(k**2)-(tl**2))/(2*fl*k)))) # cosine angle of tibia

        a = degrees(atan2(x, y_)) # coxa angle
        b = degrees(theta2 - theta) # femur angle

        c = 180 - degrees(acos(max(-1, min(1, ((fl**2)+(tl**2)-(k**2))/(2*fl*tl))))) # tibia angle from femur
        print(f"Current angle: {a, b, c} \n")
        return a,b,c
        if a > coxa_range[1] or a < coxa_range[0] or b > femur_range[1] or b < femur_range[0] or c > tibia_range[1] or c < tibia_range[0]:
            print(f"angle exceed with {a, b, c}")
...
# Run FK on those angles
predicted = FK(a, b, c)
print("FK result:", predicted)